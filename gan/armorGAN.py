...
def define_generator(latent_dim, target_img_height, target_img_width, target_img_channels):
	begin_h = int(target_img_height / 8)
	begin_w = int(target_img_width / 8)
	c = hparams.Config()

	model = Sequential()
	n_nodes = 64 * begin_h * begin_w # 128 neurons, quarter of the final image
	model.add(Dense(n_nodes, input_dim=latent_dim))
	model.add(LeakyReLU(alpha=0.2))
	model.add(Reshape((begin_h, begin_w, 64)))
	...
	model.add(LeakyReLU(alpha=0.2))
	model.add(Conv2D(target_img_channels, (13,13), activation=c.GEN_ACT, padding='same'))
	return model
